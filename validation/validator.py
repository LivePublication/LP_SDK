...
class Comparator:
        ...
        for key in expected:
            ...
            if key not in actual:
                ...
                if isinstance(expected[key], list):
                    ...
                    if len(filtered) == 0:
                        continue
                raise AssertionError(f"Key {key} not in actual: {actual}")

            if isinstance(expected[key], dict):
                self._compare_dicts(expected[key], actual[key])
                ...
                if isinstance(expected[key][0], dict):
                    ...
                    self._compare_dicts(e_dict, a_dict)
                else:
                    # Not a list of dicts - compare directly
                    assert expected[key] == actual[key], f"Value {key}: {expected[key]} does not match: {actual[key]}"
            else:
    ...
    def compare(self, actual: dict) -> bool:
        ...
        assert actual['@context'] == self.expected['@context'], "Contexts do not match\n{actual['@context']}\n{self.expected['@context']}"
        ...
        for key in self.graph_dict:
            ...
            self._compare_dicts(self.graph_dict[key], actual_graph[key])
